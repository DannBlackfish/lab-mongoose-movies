...
router.post('/signup', (req,res,next) => {
  ...
  if( !username || !email || !password) {
    res.render('users/signup', {errorMessage: "Todos los campos son obligatorios"})
    return 
  }
  const regex = /(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,}/;
  if(!regex.test(password)) {
    res.status(500).render('users/signup', {errorMessage: 'El password debe de cumplir con más de 6 caracteres, contener al menos un número, una minúscula y una mayúscula.'})
  }
  // AUTENTICACIÓN
    bcrypt
    .genSalt(saltRounds)
    ...
    .catch((error) => {
      // VALIDACIÓN DE CORREO ELECTRÓNICO
          if(error instanceof mongoose.Error.ValidationError) {
            res.status(500).render('users/signup', {errorMessage: 'El correo electrónico no tiene el formato adecuado'})
